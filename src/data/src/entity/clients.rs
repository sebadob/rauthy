...
impl Client {
    ...
    pub fn validate_resource_request(&self, resource: &str) -> Result<(), ErrorResponse> {
        ...
        if allowed.peek().is_some() {
            return if allowed.any(|r| r == resource) {
                Ok(())
            } else {
                Err(ErrorResponse::new(
                    ErrorResponseType::InvalidTarget,
        ...
        // No allow-list on the client. A dynamic client cannot declare one at
        // registration, so unless the operator seeded it from
        // `dynamic_clients.allowed_resources` (in which case the branch above matched)
        // there is no trusted source for its audiences -- reject.
        if self.is_dynamic() {
            return Err(ErrorResponse::new(
                ErrorResponseType::InvalidTarget,
...
impl From<EphemeralClientRequest> for Client {
    fn from(value: EphemeralClientRequest) -> Self {
        ...
        Self {
            ...
            // A CIMD document may not list `allowed_resources`, and it may not be ours
            // to edit. Fall back to the operator's ephemeral_clients.allowed_resources;
            // a document that declares its own resources still takes precedence.
            allowed_resources: value.allowed_resources.map(|r| r.join(",")).or_else(|| {
                let r = RauthyConfig::get()
                    .vars
                    .ephemeral_clients
                    .allowed_resources
                    .join(",");
...
impl Client {
    fn try_from_dyn_reg(
        ...
        Ok(Self {
            ...
            // A dynamically registered client cannot declare `allowed_resources`
            // itself, so seed it from config. Empty by default, which leaves these
            // clients unable to request a `resource` (see validate_resource_request).
            allowed_resources: {
                let r = RauthyConfig::get()
                    .vars
                    .dynamic_clients
