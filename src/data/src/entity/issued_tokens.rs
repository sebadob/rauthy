...
impl IssuedToken {
    ...
    pub async fn create(user_id: Option<&str>, exp: i64) -> Result<Self, ErrorResponse> {
        // Even though the chance is tiny, we want to handle PK
        // conflicts here to keep the `jti` somewhat small.
        loop {
            let slf = Self {
                jti: secure_random_alnum(16),
    ...
    async fn insert(&self) -> Result<(), ErrorResponse> {
        let sql = r#"
INSERT INTO issued_tokens (jti, user_id, exp)
VALUES ($1, $2, $3)
"#;
        if is_hiqlite() {
            DB::hql()
                .execute(sql, params!(&self.jti, &self.user_id, self.exp))
                .await?;
        } else {
            DB::pg_execute(sql, &[&self.jti, &self.user_id, &self.exp]).await?;
        }
    ...
    pub async fn revoke_for_user(user_id: String) -> Result<(), ErrorResponse> {
