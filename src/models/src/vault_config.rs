...
impl VaultConfig {
    ...
    pub async fn load_vars() -> Result<String, Box<dyn Error>> {
        ...
        let config_from_vault = Self::get().vault_source.get_config().await?;
...
impl VaultSource {
    ...
    fn build_kv_read_url(&self, path: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let url = format!("{}{}",remove_quotes(&self.addr),api_path);
        Ok(url)
    }
...
impl VaultSource {
    async fn get_config(&self) -> Result<HashMap<String, serde_json::Value>, Box<dyn std::error::Error>>  {
        self.get_secrets(&self.path).await
    }

    async fn get_certs(&self) -> Result<HashMap<String, serde_json::Value>, Box<dyn std::error::Error>>  {
        self.get_secrets(&self.path_certs).await
    }

    async fn get_secrets(&self, path: &str) -> Result<HashMap<String, serde_json::Value>, Box<dyn std::error::Error>>  {
        let url = self.build_kv_read_url(path)?;
        let client = Client::new();
        let response = client
            .get(url)
            .header("X-Vault-Token", remove_quotes(&self.token))
            .send().await?;
