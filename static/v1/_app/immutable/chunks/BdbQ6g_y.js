import { h as p, al as q, am as y, an as z, ao as D, C as w, ap as K } from "./w0HvPX0p.js";
import { i as F, c as J, d as Q, f as W, n as X, g as Z, j as m } from "./BzP2S3Z_.js";
function U(r) {
  var i, f, s = "";
...
function ir(r, i) {
  if (i) {
    ...
    if (Array.isArray(i) ? (s = i[0], t = i[1]) : s = i, r) {
      ...
      for (var v = 0; v < S; v++) {
        var g = r[v];
        if (c ? g === "/" && r[v - 1] === "*" && (c = false) : u ? u === g && (u = false) : g === "/" && r[v + 1] === "*" ? c = true : g === '"' || g === "'" ? u = g : g === "(" ? l++ : g === ")" && l--, !c && u === false && l === 0) {
          if (g === ":" && b === -1) b = v;
          else if (g === ";" || v === S - 1) {
            if (b !== -1) {
...
const N = Symbol("class"), L = Symbol("style"), V = Symbol("is custom element"), B = Symbol("is html");
function lr(r) {
  if (p) {
    var i = false, f = () => {
      if (!i) {
...
function E(r, i, f, s) {
  var t = $(r);
  p && (t[i] = r.getAttribute(i), i === "src" || i === "srcset" || i === "href" && r.nodeName === "LINK") || t[i] !== (t[i] = f) && (i === "loading" && (r[D] = f), f == null ? r.removeAttribute(i) : typeof f != "string" && G(r).includes(i) ? r[i] = f : r.setAttribute(i, f));
}
  ...
  for (const a in f) {
    ...
    if (e !== C) {
      ...
      if (S !== "$$") if (S === "on") {
        ...
        let n = a.slice(2);
        var T = m(n);
        if (F(n) && (n = n.slice(0, -7), d.capture = true), !T && C) {
          if (e != null) continue;
          r.removeEventListener(n, o[_], d), o[_] = null;
        }
        if (e != null) if (T) r[`__${n}`] = e, Q([n]);
        else {
          let H = function(Y) {
            o[a].call(this, Y);
          };
        ...
        if (e == null && !l && !j) if (u[a] = null, h === "value" || h === "checked") {
          ...
          const _ = i === void 0;
          if (h === "value") {
            let n = d.defaultValue;
            d.removeAttribute(h), d.defaultValue = n, d.value = d.__value = _ ? n : null;
          } else {
            let n = d.defaultChecked;
            d.removeAttribute(h), d.defaultChecked = n, d.checked = _ ? n : false;
          }
...
function $(r) {
  return r.__attributes ?? (r.__attributes = { [V]: r.nodeName.includes("-"), [B]: r.namespaceURI === q });
}
