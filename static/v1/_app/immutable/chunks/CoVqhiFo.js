...
import { a8 as ge, aJ as gt, a5 as je, k as T, j as x, l as P, at as _t } from "./CvlvO1XB.js";
const D = [];
function Ae(e, t = ge) {
  let n = null;
...
const U = ((_a = globalThis.__sveltekit_18bt4bh) == null ? void 0 : _a.base) ?? "/auth/v1", Pt = ((_b = globalThis.__sveltekit_18bt4bh) == null ? void 0 : _b.assets) ?? U, Ct = "1741013333219", We = "sveltekit:snapshot", Ye = "sveltekit:scroll", ze = "sveltekit:states", Ot = "sveltekit:pageurl", V = "sveltekit:history", Y = "sveltekit:navigation", ee = { tap: 1, hover: 2, viewport: 3, eager: 4, off: -1, false: -1 }, Z = location.origin;
function Je(e) {
...
const Dt = -1, Ft = -2, Vt = -3, Bt = -4, qt = -5, Gt = -6;
function Mt(e, t) {
...
let w = { branch: [], error: null, url: null }, Te = false, se = false, qe = true, X = false, G = false, rt = false, xe = false, at, k, I, j;
...
async function ct({ id: e, invalidating: t, url: n, params: r, route: o, preload: s }) {
  ...
  if (_.some(Boolean)) {
    try {
      ...
      const y = await B(p, { url: n, params: r, route: { id: e } });
  ...
  for (let p = 0; p < l.length; p += 1) if (l[p]) try {
    ...
    if (W.has(s)) return He({ error: await B(y, { params: r, url: n, route: { id: o.id } }), url: n, params: r, route: o });
    let b = ne(y), A;
    if (m == null ? void 0 : m.includes(y)) b = y.status ?? b, A = y.error;
      ...
      A = await B(y, { params: r, url: n, route: { id: o.id } });
    }
...
async function ue({ status: e, error: t, url: n, route: r }) {
  ...
  if (v.server_loads[0] === 0) try {
    ...
    (n.origin !== Z || n.pathname !== location.pathname || Te) && await q(n);
  }
  try {
...
async function M({ type: e, url: t, popped: n, keepfocus: r, noscroll: o, replace_state: s, state: i = {}, redirect_count: a = 0, nav_token: c = {}, accept: l = Be, block: d = Be }) {
  const g = j;
  j = c;
  ...
  let m = u && await ct(u);
  if (!m) {
    if (le(t, U, v.hash)) return await q(t);
    m = await ft(t, { id: null }, await B(new Ie(404, "Not Found", `Not found: ${t.pathname}`), { url: t, params: {}, route: { id: null } }), 404);
  }
  if (t = (u == null ? void 0 : u.url) || t, j !== c) return f.reject(new Error("navigation aborted")), false;
  if (m.type === "redirect") if (a >= 20) m = await ue({ status: 500, error: await B(new Error("Redirect loop"), { url: t, params: {}, route: { id: null } }), url: t, route: { id: null } });
  else return await Pe(new URL(m.location, t).href, {}, a + 1, c), false;
  else m.props.page.status >= 400 && await N.updated.check() && (await tt(), await q(t));
  ...
  !r && !L && _n(), qe = true, m.props.page && Object.assign(S, m.props.page), X = false, e === "popstate" && st(I), f.fulfil(void 0), K.forEach((p) => p(f.navigation)), N.navigating.set(z.current = null);
}
async function ft(e, t, n, r) {
  return e.origin === Z && e.pathname === location.pathname && !Te ? await ue({ status: r, error: n, url: e, route: t }) : await q(e);
}
function un() {
  let e, t;
...
async function gn(e, { status: t = 200, error: n, node_ids: r, params: o, route: s, server_route: i, data: a, form: c }) {
  ...
  try {
    ...
    g = await ue({ status: ne(f), error: await B(f, { url: l, params: o, route: s }), url: l, route: s }), e.textContent = "", u = false;
  }
