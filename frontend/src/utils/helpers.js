...
export const redirectToLogin = (state) => {
	getPkce(43, (error, {challenge, verifier}) => {
		if (!error) {
			...
			const s = state || 'admin';
			window.location.href = `${AUTH_ENDPOINT}?client_id=${CLIENT_ID}&redirect_uri=${window.location.origin}${REDIRECT_URI}&response_type=code&code_challenge=${challenge}&code_challenge_method=S256&scope=openid+profile+email&nonce=${nonce}&state=${s}`;
		}
	});
};
