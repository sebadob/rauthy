<script lang="ts">
  ...
  let authorizeUrl = $derived(
    IS_DEV ? '/auth/v1/dev/authorize' : '/auth/v1/oidc/authorize',
  );
  ...
  let stateEncoded = $derived(
    stateParam ? encodeURIComponent(stateParam) : undefined,
  );
  ...
  async function onSubmit(form?: HTMLFormElement, params?: URLSearchParams) {
    ...
    let res = await fetchPost<
      undefined | WebauthnLoginResponse | ToSAwaitLoginResponse
  ...
  async function handleAuthRes(
    ...
    if (res.status === 202) {
      ...
      if (body && 'user_id' in body && 'code' in body) {
        ...
        console.error(
          'did not receive a proper WebauthnLoginResponse after HTTP200',
        );
      }
